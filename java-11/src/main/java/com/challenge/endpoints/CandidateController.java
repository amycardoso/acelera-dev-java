...
public class CandidateController {
    ...
    public List<CandidateDTO> findByCompanyOrAcceleration(
        ...
        if(companyId != null) candidates = candidateMapper.map(candidateService.findByCompanyId(companyId)); 

        if(accelerationId != null) candidates = candidateMapper.map(candidateService.findByAccelerationId(accelerationId));
